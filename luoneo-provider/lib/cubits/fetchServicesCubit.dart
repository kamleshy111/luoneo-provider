...
class FetchServicesCubit extends Cubit<FetchServicesState> {
  ...
  Future<void> fetchServices({String? order, String? sort}) async {
    try {
      emit(FetchServicesInProgress());
      final DataOutput<ServiceModel> result = await _serviceRepository
          .fetchService(offset: 0, order: order, sort: sort);
  ...
  Future<void> fetchMoreServices({String? order, String? sort}) async {
    try {
  ...
  Future<void> searchService(String query, String? order, String? sort) async {
    try {
      emit(FetchServicesInProgress());
